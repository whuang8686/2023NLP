...
fp = open("./test_result_3.csv", mode='w')
...
df1 = pd.read_csv('test_result_1.csv', sep="|", on_bad_lines='skip')
df2 = pd.read_csv('test_result_2.csv', sep="|", on_bad_lines='skip')
df3 = pd.read_csv('test_result_3.csv', sep="|", on_bad_lines='skip')
...
sorted_df = merged_df.sort_values(['test_query', 'score'], ascending=False)  # 根据需要替换'column_name'
sorted_df.to_csv('sorted_data1.csv', sep="|", index=False)


top5_df = sorted_df.groupby('test_query').head(5)


# 可选：将排序结果保存为新的CSV文件
top5_df.to_csv('sorted_data.csv', sep="|", index=False)

file_path2 = os.path.join(folder_path, "sorted_data.csv")
# 讀取 JSONL 檔案並存入 DataFrame
...
for index, row in test_out_df.iterrows():
      ...
      with jsonlines.open(folder_path+'test.jsonl', mode="a") as writer:
        writer.write(test_out_list)
#print (test_out_list)
